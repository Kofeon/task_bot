...
async def boost_task(call: types.CallbackQuery):
    ...
    await redis.set("price", call.data[:-1])
    ...
    await bot.edit_message_text(message_id=call.message.message_id,
                                ...
                                text=f"Тип: {type}\n"
                                     ...
                                     f"а затем потвердите буст.", reply_markup=markup
                                )

async def confirm_boost(call: types.CallbackQuery):
    ...
    cur.execute('UPDATE telegram_channel_tasks SET executed_quantity = (executed_quantity + ?), who_did = (who_did || ?) WHERE task_id = ?',
...
for i in telegram_channel_tasks:
    ...
    if type[0] == "b":
        boost_ids.append(f"{task_id}b")

def reg(dp):
    dp.register_callback_query_handler(boost_task, text=boost_ids)
    dp.register_callback_query_handler(confirm_boost, text="confirm")